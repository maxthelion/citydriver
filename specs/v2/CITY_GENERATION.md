...
4. **Place in block** — standard placement with overlap detection and heightmap grounding. Each building samples the heightmap at its center and four corners, takes the **maximum** height, and places its base there. This ensures the ground floor door is accessible at the highest terrain point. An extra depth (~8 units) is added below ground level on every building body so the lower corners are buried into the slope rather than leaving a visible gap.
...
6. Ground building bases on heightmap (max of center + 4 corners, extra depth buries low side)
```
