#!/usr/bin/env bun
/**
 * Experiment 007k2 — Distance-spaced, elevation-snapped junction points.
 *
 * Problem with 007k: junction points were placed at every 2m elevation crossing
 * along each cross street. On steep terrain this produced points every few metres
 * horizontally — far too dense (693 parallels reported).
 *
 * Fix: walk each cross street measuring HORIZONTAL arc-length distance. Every
 * PARALLEL_SPACING (35m), note the current position, read the elevation, snap it
 * to the nearest ELEV_INTERVAL (2m) contour, then search ±SNAP_SEARCH cells along
 * the cross street for the position where elevation is closest to the snapped value.
 * Record that position with the snapped elevation as the junction key.
 *
 * This means:
 *  - Points are ~35m apart horizontally regardless of slope.
 *  - Each point's elevation is quantised to 2m intervals.
 *  - Adjacent cross streets will find points at the same quantised elevation key
 *    → continuous contour parallels without jogs.
 *  - On steep ground successive points may snap to adjacent 2m levels, giving
 *    slight spacing variation; on gentle ground multiple points collapse to the
 *    same key (only one recorded, the first).
 */
...
const outDir = process.argv[5] || 'experiments/007k2-output';
...
// ===== Distance-snapped junction layout =====
...
const ELEV_INTERVAL   = 2;     // metres — quantisation interval for elevation snapping
const SNAP_SEARCH     = 10;    // cells — search radius when refining snapped position
...
for (let fi = 0; fi < faces.length; fi++) {
  ...
  for (let ctOff = firstCt; ctOff <= maxCt + 1e-6; ctOff += CROSS_SPACING) {
    ...
    // Every PARALLEL_SPACING metres of horizontal distance, note position,
    // read elevation, snap to nearest ELEV_INTERVAL multiple, then search
    // ±SNAP_SEARCH samples along the street to find the position whose
    // elevation is closest to the snapped target.
    //
    // junctionMap: snappedElevKey (integer) → {x, z, elevation}
    // Using an integer key (snappedElev / ELEV_INTERVAL rounded) ensures
    // adjacent cross streets share the same key for the same contour level.
    // ------------------------------------------------------------------
    const junctionMap = new Map(); // key = snappedElev / ELEV_INTERVAL (integer)

    // Re-sample bestRun at half-cell steps for a dense profile.
    // Each sample stores its arc-length index so we can search ±SNAP_SEARCH.
    ...
    for (let si = 0; si < profile.length; si++) {
      ...
      // Current elevation at this position
      const cgx2 = profile[si].cgx;
      const cgz2 = profile[si].cgz;
      if (cgx2 < 0 || cgx2 >= W || cgz2 < 0 || cgz2 >= H) continue;
      const currentElev = elev.get(cgx2, cgz2);

      // Snap to nearest ELEV_INTERVAL multiple
      const snappedElev = Math.round(currentElev / ELEV_INTERVAL) * ELEV_INTERVAL;
      const elevKey = Math.round(snappedElev / ELEV_INTERVAL); // integer key

      // Only record first occurrence of each elevation key on this street
      if (junctionMap.has(elevKey)) continue;

      // Search ±SNAP_SEARCH samples for the position whose elevation is
      // closest to snappedElev. This refines the junction onto the contour.
      let bestIdx = si;
      let bestDiff = Math.abs(currentElev - snappedElev);

      const lo = Math.max(0, si - SNAP_SEARCH);
      const hi = Math.min(profile.length - 1, si + SNAP_SEARCH);
      for (let sj = lo; sj <= hi; sj++) {
        if (sj === si) continue;
        const sjCgx = profile[sj].cgx;
        const sjCgz = profile[sj].cgz;
        if (sjCgx < 0 || sjCgx >= W || sjCgz < 0 || sjCgz >= H) continue;
        const diff = Math.abs(elev.get(sjCgx, sjCgz) - snappedElev);
        if (diff < bestDiff) {
          bestDiff = diff;
          bestIdx = sj;
        }
      ...
      const best = profile[bestIdx];
      junctionMap.set(elevKey, { x: best.wx, z: best.wz, elevation: snappedElev });
    }
  ...
  // ------------------------------------------------------------------
  // Step 5: Connect matching elevation keys between adjacent cross streets.
  // Same logic as 007k — shared key → continuous contour parallel.
  // ------------------------------------------------------------------
...
// Parallel streets (cyan, 1px) — distance-spaced elevation-snapped contour followers
