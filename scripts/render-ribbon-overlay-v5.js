#!/usr/bin/env bun
/**
 * Experiment 007s7 — Straightness-weighted triangle edge selection:
 *
...
const outDir = process.argv[5] || 'experiments/007s7-output';
...
// Build 3 triangle edges, compute lengths
const triEdges = [];
for (let i = 0; i < 3; i++) {
  const p1 = corners[i], p2 = corners[(i + 1) % 3];
  const dx = p2.x - p1.x, dz = p2.z - p1.z;
  const len = Math.sqrt(dx * dx + dz * dz);

  // Collect boundary points near this edge for the points array
  const edgePts = [];
  for (const bp of b) {
    const t = ((bp.x - p1.x) * dx + (bp.z - p1.z) * dz) / (len * len);
    if (t < -0.05 || t > 1.05) continue;
    const projX = p1.x + t * dx, projZ = p1.z + t * dz;
    const dist = Math.sqrt((bp.x - projX) ** 2 + (bp.z - projZ) ** 2);
    if (dist < cs * 15) edgePts.push({ x: bp.x, z: bp.z, t });
  }
  edgePts.sort((a, b2) => a.t - b2.t);

  triEdges.push({
    points: edgePts.length >= 2 ? edgePts : [p1, p2],
    length: len,
...
// Compute straightness for each edge: average distance of boundary points to the chord
for (const edge of triEdges) {
  const dx = edge.end.x - edge.start.x, dz = edge.end.z - edge.start.z;
  const len = edge.length;
  let totalDev = 0;
  for (const pt of edge.points) {
    const t = ((pt.x - edge.start.x) * dx + (pt.z - edge.start.z) * dz) / (len * len);
    const projX = edge.start.x + t * dx, projZ = edge.start.z + t * dz;
    totalDev += Math.sqrt((pt.x - projX) ** 2 + (pt.z - projZ) ** 2);
  }
  edge.avgDev = edge.points.length > 0 ? totalDev / edge.points.length : 0;
  // Straightness: 1 = perfectly straight, 0 = very curvy
  edge.straightness = Math.max(0, 1 - edge.avgDev / (cs * 10));
  edge.score = edge.length * edge.straightness;
}

// Sort by score (straightness × length), pick two best with angle > 15°
triEdges.sort((a, b2) => b2.score - a.score);
let edges = [triEdges[0]];
for (let i = 1; i < triEdges.length; i++) {
  const diff = Math.abs(triEdges[i].angle - edges[0].angle);
  const norm = diff > 90 ? 180 - diff : diff;
  if (norm > 15) { edges.push(triEdges[i]); break; }
}
for (const e of triEdges) {
  console.log(`  Edge ${e.angle.toFixed(0)}° len=${e.length.toFixed(0)}m avgDev=${e.avgDev.toFixed(1)}m straight=${e.straightness.toFixed(2)} score=${e.score.toFixed(0)}`);
}
...
if (hasS2) {
  ...
  // Anchor road A: thick white line
  const startA = edges[0].start, endA = edges[0].end;
  thickLine(pixels, cropW, cropH,
    Math.round((startA.x - ox) / cs) - minGx, Math.round((startA.z - oz) / cs) - minGz,
    Math.round((endA.x - ox) / cs) - minGx, Math.round((endA.z - oz) / cs) - minGz,
    255, 255, 255, 2);

  // Anchor road B: thick bright yellow line
  const startB = edges[1].start, endB = edges[1].end;
  thickLine(pixels, cropW, cropH,
    Math.round((startB.x - ox) / cs) - minGx, Math.round((startB.z - oz) / cs) - minGz,
    Math.round((endB.x - ox) / cs) - minGx, Math.round((endB.z - oz) / cs) - minGz,
    255, 255, 0, 2);
}
