#!/usr/bin/env bun
/**
 * Experiment 007s3 — Overlay of two approaches on the SAME zone:
 *
...
const outDir = process.argv[5] || 'experiments/007s3-output';
...
// Sort by score descending and pick top two that form a meaningful angle
nearbyRoads.sort((a, b) => b.score - a.score);

let edges = [];
if (nearbyRoads.length >= 1) {
  edges.push(nearbyRoads[0]);
  // Find second road that forms angle > 15° with the first
