#!/usr/bin/env bun
/**
 * render-ridge-fullmap-v2.js — Whole-map terrain faces (zone-independent).
 *
 * Faces are a parallel system to zones — they segment the entire buildable
 * terrain by gradient direction, independent of zone boundaries.
 *
 * Renders: elevation base, water, semi-transparent face colors, face boundaries
 * (white), zone boundaries (yellow), roads (grey).
 *
 * Usage: bun scripts/render-ridge-fullmap-v2.js <seed> <gx> <gz> [outDir]
 */
...
const outDir = process.argv[5] || 'experiments/019c-output';
...
// === Segment entire terrain (zone-independent) ===
...
for (let i = 0; i < faces.length; i++) {
  const f = faces[i];
  console.log(`  Face ${i}: ${f.cells.length} cells, slopeDir=(${f.slopeDir.x.toFixed(3)}, ${f.slopeDir.z.toFixed(3)})`);
}
...
// Layer 6: Zone boundaries (yellow, 2px thick)
...
// === Write output ===
const header = `P6\n${W} ${H}\n255\n`;
const basePath = `${outDir}/ridge-terrain-seed${seed}`;
