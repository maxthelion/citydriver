#!/usr/bin/env bun
/**
 * Experiment 007i — Gradient-direction cross streets.
 *
 * Improvement over 007h: instead of using arbitrary top/bottom edge
 * correspondences for cross streets, compute each face's steepest gradient
 * direction from its cells' elevation data and sweep cross streets along
 * that direction (uphill). Parallel streets follow the contour direction.
 *
 * Algorithm per face:
 *  1. Compute average gradient (dx, dz) from finite differences of elevation.
 *  2. Normalise → gradient unit vector (uphill direction).
 *  3. Contour direction = perpendicular to gradient.
 *  4. Sweep cross street lines at CROSS_SPACING intervals along the contour axis.
 *  5. Each cross street runs in the gradient direction; clip to face cells.
 *  6. Mark PARALLEL_SPACING points along each cross street.
 *  7. Connect same-offset points between adjacent cross streets → parallel streets.
 */
...
const outDir = process.argv[5] || 'experiments/007i-output';
...
// Pick same zone as 007h — medium-large, near centre, has slope data
...
// ===== Terrain face segmentation (same as 007h) =====
...
for (let fi = 0; fi < faces.length; fi++) {
  ...
  //
  // For each interior cell we estimate the elevation gradient using
  // central differences (or one-sided where neighbours are outside the
  // face). We then average all per-cell gradients to get the face's
  // dominant uphill direction.
  // ------------------------------------------------------------------
  ...
  if (gradMag < 1e-6) {
    // Flat face — fall back to zone slopeDir if available
    if (zone.slopeDir) {
      gradX = zone.slopeDir.x;
      gradZ = zone.slopeDir.z;
    } else {
      continue; // Skip truly flat faces
    }
  } else {
    gradX /= gradMag;
    gradZ /= gradMag;
  ...
  //
  // Project every cell's world position onto the contour axis and find
  // the min/max. We will sweep cross street lines at CROSS_SPACING
  // intervals along this axis.
  // ------------------------------------------------------------------
  ...
  //
  // Each cross street is a line that runs in the gradient direction.
  // We find which cells the line passes through and clip to the face.
  // Instead of precise polygon clipping we walk cell-by-cell: start at
  // the face centroid along the gradient, extend both ways until we
  // leave the face.
  // ------------------------------------------------------------------

  const crossStreetPoints = []; // [{contourOffset, points: [{x,z,gradOffset}]}]

  // Snap first sweep position to a multiple of CROSS_SPACING for alignment
  ...
  for (let ctOff = firstCt; ctOff <= maxCt + 1e-6; ctOff += CROSS_SPACING) {
    ...
    // Walk along gradient direction, step = half a cell
    const step = cs * 0.5;
    const reach = (maxGr - minGr) + cs * 2;
    const nSteps = Math.ceil(reach / step);

    // Collect world-space points that fall inside the face,
    // grouped into contiguous runs.
    const inFacePoints = []; // {wx, wz, grOff, inFace}
    ...
    // Step 4: Mark PARALLEL_SPACING points along the gradient offset range.
    // Anchor to multiples of PARALLEL_SPACING so adjacent cross streets share offsets.
    ...
    for (let grOff = firstGr; grOff <= grMax + 1e-6; grOff += PARALLEL_SPACING) {
      // Interpolate world position along the run
  ...
  // Step 5: Connect same-grOff points on adjacent cross streets → parallel streets.
...
// ===== Render (cropped, same style as render-ribbon-faces-zoomed.js) =====
...
// Cross streets (magenta, 1px) — should run straight uphill
...
// Parallel streets (cyan, 1px) — should follow contours
