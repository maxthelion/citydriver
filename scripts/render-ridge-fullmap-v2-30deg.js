#!/usr/bin/env bun
/**
 * render-ridge-fullmap-v2-30deg.js — 30° tolerance variant.
 *
 * Same as render-ridge-fullmap-v2.js but with 30° direction tolerance
 * instead of 60°, producing more terrain faces.
 *
 * Usage: bun scripts/render-ridge-fullmap-v2-30deg.js <seed> <gx> <gz> [outDir]
 */
...
const outDir = process.argv[5] || 'experiments/019d-output';
...
// === Segment entire terrain with 30° tolerance ===
console.log('Segmenting terrain (30° tolerance)...');
const { faces, ridgeCells } = segmentTerrainV2(map, { dirTolerance: Math.PI / 6 });
console.log(`Terrain faces: ${faces.length}, boundary cells: ${ridgeCells.size}`);
for (let i = 0; i < faces.length; i++) {
  const f = faces[i];
  // Compute elevation range and slope stats within face
...
// === Write output ===
const header = `P6\n${W} ${H}\n255\n`;
const basePath = `${outDir}/ridge-terrain-30deg-seed${seed}`;
