 ...
 *
 * Problem with 007i/j/k: cross streets follow the steepest gradient and meet anchor
 * roads at steep angles instead of right angles. Real streets prefer perpendicular
 * junctions with main roads.
 *
 * Fix: for each terrain face, find the nearest arterial/collector road segment to the
 * face centroid. Compute the road's direction and its perpendicular. Blend the gradient
 * direction with the road perpendicular:
 *
 *   crossDir = normalize(0.7 * gradientDir + 0.3 * roadPerpendicularDir)
 *
 * Algorithm per face:
 *  1. Compute average gradient direction (same as 007i).
 *  2. Find nearest arterial/collector road segment to the face centroid.
 *  3. Compute road direction and its perpendicular.
 *  4. Blend: crossDir = normalize(0.7 * gradientDir + 0.3 * roadPerpendicularDir).
 *  5. Use crossDir for cross streets; contour direction = perpendicular to crossDir.
 *  6. Sweep cross streets along the contour axis (same as 007i).
 *  7. Mark PARALLEL_SPACING points along each cross street.
 *  8. Connect same-offset points between adjacent cross streets → parallel streets.
 *
 * Render: magenta cross (1px), cyan parallel (1px), yellow boundary,
 *         nearest road segment in bright white (3px).
 */
...
const outDir = process.argv[5] || 'experiments/007l-output';
...
// Build a flat list of road segments with direction info for fast lookup
 ...
 * Returns { seg, dist, closestX, closestZ } or null.
 */
function findNearestAnchorSegment(px, pz) {
  if (anchorSegments.length === 0) return null;
  let bestSeg = null, bestDist = Infinity, bestCx = 0, bestCz = 0;
  ...
  return bestSeg ? { seg: bestSeg, dist: bestDist, closestX: bestCx, closestZ: bestCz } : null;
}
...
// ===== Road-blend cross layout =====
const CROSS_SPACING = 90;     // metres between cross streets (along contour)
const PARALLEL_SPACING = 35;  // metres between parallel streets (along gradient)
const MIN_STREET_LEN = 20;    // metres — skip degenerate segments
const BLEND_GRADIENT = 0.7;   // weight for gradient direction
const BLEND_ROAD    = 0.3;    // weight for road perpendicular direction

const allCross = [];
const allParallel = [];
const faceTints = [[60,100,60],[60,60,100],[100,80,50],[80,60,100],[60,100,100],[100,60,80]];

// Store nearest road segment per face for rendering
...
for (let fi = 0; fi < faces.length; fi++) {
  ...
  // ------------------------------------------------------------------
  // Step 2: Compute face centroid in world coords.
  // ------------------------------------------------------------------
  let cxSum = 0, czSum = 0;
  ...
  // ------------------------------------------------------------------
  // Step 3: Find nearest anchor road segment; compute road perpendicular.
  // ------------------------------------------------------------------
  ...
  if (nearestResult) {
    ...
    // Road direction: (seg.dirX, seg.dirZ)
    // Road perpendicular: (-seg.dirZ, seg.dirX) — points 90° to road
    const perpX = -seg.dirZ;
    const perpZ =  seg.dirX;

    // Choose perpendicular direction that most aligns with gradient
    // (so we don't flip toward anti-gradient)
    const dot = gradX * perpX + gradZ * perpZ;
    const rpX = dot >= 0 ? perpX : -perpX;
    const rpZ = dot >= 0 ? perpZ : -perpZ;

    // Blend gradient with road perpendicular
    const blendX = BLEND_GRADIENT * gradX + BLEND_ROAD * rpX;
    const blendZ = BLEND_GRADIENT * gradZ + BLEND_ROAD * rpZ;
    ...
    faceNearestSegs.push({ fi, seg });
    console.log(`  Face ${fi}: nearest road dist=${nearestResult.dist.toFixed(1)}m hierarchy=${seg.road.hierarchy} blend=(${crossX.toFixed(3)},${crossZ.toFixed(3)})`);
  } else {
    // No anchor roads — fall back to pure gradient
    crossX = gradX;
    crossZ = gradZ;
    faceNearestSegs.push(null);
    console.log(`  Face ${fi}: no anchor roads, using pure gradient`);
  }

  // Cross direction: (crossX, crossZ)
  // Contour direction (perpendicular to cross): (-crossZ, crossX)
  const ctX = -crossZ, ctZ = crossX;

  // ------------------------------------------------------------------
  // Step 4: Find face extent along the contour direction.
  // ------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------
  // Step 5: Sweep cross streets at CROSS_SPACING along the contour axis.
  // ------------------------------------------------------------------
  ...
  for (let ctOff = firstCt; ctOff <= maxCt + 1e-6; ctOff += CROSS_SPACING) {
    ...
    // Step 6: Mark PARALLEL_SPACING points along the cross street.
  ...
  // Step 7: Connect same-grOff points on adjacent cross streets → parallel streets.
...
// Nearest anchor road segments per face — bright white (3px)
for (const entry of faceNearestSegs) {
  if (!entry) continue;
  const { seg } = entry;
  // Draw the full segment in bright white, 3px wide
  ...
  bresThick(pixels, cropW, cropH, x0, z0, x1, z1, 255, 255, 255, 3);
}
...
// Parallel streets (cyan, 1px) — follow contour of blended direction
