...
The [[terrain-face-streets]] page describes a more sophisticated approach that identifies each face's top edge, bottom edge, and side edges explicitly. Cross streets would be generated by subdividing the top and bottom edges at matched intervals, then connecting corresponding points. This would produce more consistent cross streets and better junction alignment than the current contour-sweep approach.

The current algorithm is a stepping stone toward that design. The known issues (junction matching on irregular faces, face coverage gaps) would be addressed by the edge-subdivision approach.
